...
<!--            <div class="box-header with-border">
              ...
              <div class="box-body">
                  <?php  foreach($category_data as $data){?>
                    <button type="submit" class="btn btn-success btn-lg cat" id="btn_submit_<?php echo $data->cat_id;?>" name="btn_submit" style="background-color:#00ffc3;color:black;margin-bottom:5px;font-weight:bold;width:96px;overflow: hidden;text-overflow: ellipsis;white-space:normal;margin-right:-4px" onclick="get_items(<?php echo $data->cat_id;?>,0)">{{$data->cat_name}}</button>
                    <span class="info-box-number cat_id" style="display:none;"><h2>{{$data->cat_id}}</h2></span>
                  
                  <?php }$all=0; ?>
                    <button type="submit" class="btn btn-success btn-lg" id="btn_submit1" name="btn_submit" style="background-color:#00ffc3;color:black;margin-top: -5px;font-weight:bold;" onclick="get_items(<?php echo $all;?>),0">All Items</button>
                    <br/>
                    <br/>
                    <div id="item_data">
                ...
                {{ csrf_field() }}
                 <div class="box box-default">
                    ...
                    @endif
                        ...
                        <table width="100%">
                            <tr>
                                <td class='print_1'><b>Bill No: {{@$bill_data->bill_no}}</b></td>
                                <td class='print_1' style="text-align: right;"><b>Date: <?php echo date('Y-m-d');?>    <?php echo date("h:i:s");?></b></td>
                            </tr>
                        </table>
                    </div>
...
//     window.print();
	  ...
	  // window.print();
        ...
        var printWindow = window.open(windowUrl, windowName, 'left=50000,top=50000,width=800,height=600');
        printWindow.document.write(printContent.innerHTML);
        printWindow.document.write('<html><head><style>th { font-size: 20px }.print{ font-size: 20px }.print_1{ font-size: 18px }<style></head><body>');
        printWindow.document.close();
        printWindow.focus();
        printWindow.print();
        printWindow.close();
...
if(flag==0)
{
    ...
    $(".row1:last").before("<tr class='input_fields_wrap'>\n\
                    ...
                    <td style='text-align:center;'><textarea name='stoppage[" + i + "][1]' class='form-control item_name print' cols='10' style='border:none;resize:none;width:200px;height:auto;background-color:#ffffff;' readonly>" + item + "</textarea></td>\n\
                    ...
                    <td style='text-align:center;'><input type='text' name='stoppage[" + i + "][4]' class='form-control item_disc print' id='item_disc_" + i + "' value='" + disc + "' style='border:none;width:50px;text-align:center;background-color:#ffffff;' readonly/></td>\n\
                    <td style='text-align:center;'><input type='text' name='stoppage[" + i + "][5]' class='form-control item_tax print' id='item_tax_" + i + "' value='" + tax + "' style='border:none;width:50px;text-align:center;background-color:#ffffff;' readonly/></td>\n\
